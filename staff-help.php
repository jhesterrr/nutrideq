...
<head>
    ...
    <style>
        /* IMMORTAL CLINICAL UI - OVERRIDE GHOSTING */
        ...
        .page-container { max-width: 1400px; margin: 0 auto; width: 100%; height: 100%; position: relative; }

        /* THE MESSAGING ENGINE */
        .messaging-wrapper { 
            display: flex !important; opacity: 1 !important; visibility: visible !important; 
            gap: 20px; height: calc(100vh - 100px); width: 100% !important; animation: none !important;
        }
        .msg-sidebar { 
        ...
        .contact-avatar { width: 40px; height: 40px; border-radius: 50%; background: #eee; display: flex; align-items: center; justify-content: center; font-weight: 700; color: #666; }

        .msg-container { 
            flex: 1; background: white; border-radius: 20px; display: flex; flex-direction: column; overflow: hidden; 
            box-shadow: 0 4px 25px rgba(0,0,0,0.05); border: 1px solid #eee;
        }
        ...
        .message-bubble { padding: 12px 18px; border-radius: 18px; font-size: 0.95rem; line-height: 1.5; color: #333; }
        .message-wrapper.sent .message-bubble { background: #2E8B57; color: white; border-bottom-right-radius: 4px; }
        .message-wrapper.received .message-bubble { background: #f1f3f5; border-bottom-left-radius: 4px; }
        ...
        .chat-input { flex: 1; border: none; background: transparent; padding: 8px 12px; outline: none; font-size: 0.95rem; resize: none; }
        .send-btn { background: #2e8b57; color: white; border: none; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; transition: 0.2s; }
        ...
        @media (max-width: 1024px) {
            ...
            .messaging-wrapper { flex-direction: column; height: auto; }
            .msg-sidebar, .msg-container { width: 100%; height: 500px; }
        }
    </style>
</head>
<body>
                            ...
                            <?php foreach ($threads as $thread): ?>
                                <div class="contact-item <?= ($selected_thread_id == $thread['id']) ? 'active' : '' ?>" onclick="window.location.href='?thread_id=<?= $thread['id'] ?>'">
                                    ...
                                    <div class="contact-info">
                                        <div style="font-weight:600; color:#333;"><?= htmlspecialchars($thread['title']) ?></div>
                                        <div style="font-size:0.8rem; color:#999;"><?= ucfirst($thread['status']) ?></div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
                ...
                <div class="modal-body">
                    ...
                    <form method="POST">
                        <div style="margin-bottom:15px;">
                            ...
                            <input type="text" name="thread_title" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:10px;" required>
                        </div>
                        <div style="margin-bottom:15px;">
                            <label style="display:block; font-weight:600; margin-bottom:5px;">Clinical Admins</label>
                            <div style="max-height:100px; overflow-y:auto; padding:5px; border:1px solid #f0f0f0; border-radius:10px;">
                        ...
                        <div style="margin-bottom:15px;">
                            ...
                            <textarea name="initial_message" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:10px;" rows="3" required></textarea>
                        </div>
